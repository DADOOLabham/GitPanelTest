...
#         host = os.getenv('UNSUPERVISED_MYSQL_HOST', 'localhost'),
# 	user = os.getenv('UNSUPERVISED_MYSQL_USER', 'intern1'),
# 	password = os.getenv('UNSUPERVISED_MYSQL_PASSWORD', '1234'),
# 	database = os.getenv('UNSUPERVISED_MYSQL_DB', 'intern1UnsupervisedDb')
#     )
#     return connection
...
def visualize_clusters(df, num_clusters):
    ...
    st.subheader("Cluster-wise Total Transaction Amount - Bar Chart")
    cluster_sums = df.groupby('Cluster')['Transaction_Amount'].sum().sort_index()
    plt.figure(figsize=(8, 5))
    cluster_sums.plot(kind='bar', color=plt.cm.Paired.colors)
    plt.title("Total Transaction Amount by Cluster")
    plt.xlabel("Cluster")
...
def main():
    ...
    if uploaded_file is not None:
        ...
        clustered_df, kmeans = perform_kmeans(df, num_clusters=3)
