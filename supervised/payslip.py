...
#         # Skip if amount is NaN or inf
#         if isinstance(amount, float) and (math.isnan(amount) or math.isinf(amount)):
#             print(f"Skipping invalid amount at row {index}: {amount}")
#             continue
...
def visualize_pie_chart(data):
    ...
    # Drop rows with NaN or non-positive values in 'Amount'
    filtered_data = data.dropna(subset=["Amount"])
...
def main():
    ...
    if uploaded_file is not None:
        ...
        if earnings_data:
            ...
            csv_path = "/home/intern1/OCR_Project/csv/temp_payslip.csv"
            save_to_csv(earnings_data, csv_path)
